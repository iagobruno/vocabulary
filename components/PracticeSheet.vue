...
const canPractice = computed(() => store.viewedWords.size >= 5);
...
function generateQuestions() {
    ...
    const words = lastSeenWords
        .map((n) => allWordsMap.get(n))
    ...
    state.questions = selected.map((w) => {
        ...
        while (wrongMeanings.length < 2) {
            ...
            if (
                random.word !== w.word &&
                !wrongMeanings.includes(random.meaning)
            ) {
...
<template>
    <ModalSheet
        :show="show"
        @close="emit('close')"
        modalClass="h-[85dvh]! overflow-x-hidden"
    >
        <div
            v-if="!state.started"
        ...
        >
            ...
            <h2 class="text-4xl font-lora font-medium mb-4">
                Hora de praticar
            </h2>
            <p class="text-gray-300 text-sm/6 max-w-xs">
                Reveja as 10 palavras que você acabou de ver para fixá-las na
                memória.
            </p>
            <Button
                :disabled="!canPractice"
                @click="startPractice"
                class="mt-20 w-full py-3 max-w-[90%]"
                :class="
                    canPractice
                        ? 'bg-white! text-black'
            ...
            <p v-if="!canPractice" class="text-xs text-white/50 mt-3">
                Veja pelo menos 5 palavras para praticar.
            </p>
        </div>
        ...
        >
            <p
                ...
                :class="scorePercent >= 70 ? 'text-3xl' : 'text-xl'"
            >
                {{
            ...
            <p class="text-gray-300 text-sm mt-8 mb-2">
                Você acertou {{ state.correctCount }} de
            ...
            <div v-if="scorePercent < 70" class="mt-20 text-center text-xs">
                ...
                <Button
                    @click="restart"
                    class="bg-white! text-black text-lg py-3 px-8!"
                >
                    Recomeçar
                </Button>
            </div>
            ...
            <Button
                v-else
                @click="emit('close')"
                class="mt-20 bg-white! text-black py-3 px-8!"
            >
        ...
        <div v-else-if="currentQuestion">
            <h3 class="text-[1.06rem] font-medium text-center opacity-90 pt-6">
                Hora de praticar
            </h3>
            <Transition name="slide" mode="out-in">
                <div :key="state.currentIndex" class="px-6 pt-14">
                    ...
                    <p class="text-4xl font-lora text-center mb-14">
